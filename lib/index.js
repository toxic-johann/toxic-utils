 ...
 * Released under MIT
 * Built ad Sat Jun 01 2019 23:56:18 GMT+0800 (China Standard Time)
 */
...
function genTraversalHandler(fn, setter) {
  ...
  function recursiveFn(source, target, key) {
    if (lodashEs.isArray(source) || lodashEs.isPlainObject(source)) {
      target = !lodashEs.isObject(target) ? lodashEs.isPlainObject(source) ? {} : [] : target;
...
function getDeepProperty(obj, keys, _a) {
  ...
  if (lodashEs.isString(keys)) {
    keys = keys.split(".");
  }

  if (!lodashEs.isArray(keys)) {
